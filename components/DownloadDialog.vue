      ...
      <v-card light>
        ...
        <v-card-actions>
          ...
          <v-btn
            dark
            color="primary"
            @click="onClose" >Cancel</v-btn>
          <v-btn
            dark
            color="primary"
            @click="downloadData" >Download</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  methods: {
    setInitDate() {
      ...
      this.queryData.range_start = now.add(-1, 'M').format('YYYY-MM-DD')
      this.queryData.range_end = now.add(1, 'd').format('YYYY-MM-DD')
    },
    onClose() {
      this.clearForm()
      this.$emit('onClose')
      ...
      if (this.radios === '1') {
        ...
        try {
          ...
          for (let key in this.queryData) {
            if (
              this.queryData.hasOwnProperty(key) &&
          ...
          if (resp.data && resp.data.length > 0) {
            this.csvExport(this.model + 's', resp.data)
          } else {
